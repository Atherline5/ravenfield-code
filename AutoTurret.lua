    ...
    if self.hasTarget and not self.gameObject.GetComponent(Vehicle).isBurning and self.currentTarget ~= nil then
        ...
        if self.gunLead then
            ...
            if self.currentTarget.activeVehicle == nil and self.targetActors == true then
                ...
                if calcTraj1 ~= 0 then
                    --print("cakj")
                    local trajectoryHeight = Mathf.Tan(calcTraj1 * Mathf.Deg2Rad) * Vector3.Distance(self.rotateableTurret.transform.position, self.currentTarget.position)
                    --print(trajectoryHeight)
                    if self.currentTarget.isParachuteDeployed then
                        trajectoryHeight = Vector3.Distance(self.rotateableTurretGun.transform.position, self.currentTarget.position)
                        --print(trajectoryHeight)
                    end
